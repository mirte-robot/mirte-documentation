...
------------------------------------------------------------------------------------
Currently this computer would need to have a USB (to connect to the microcontroller) and wifi (e.g. through a dongle). You might then try to install Zoef from source. You will have the best change with a 32bit Ubuntu distribution installed.
...
I am not able to connect to zoef.local.
---------------------------------------
This might be a problem related to Windows/Android. If you are running Linux or MacOS this should work. First make sure that you are on the same network. Also check if you can ping zoef.local. Also make sure that you are trying to connect to http://zoef.local/ (the http:// in front, and / at the end might be needed).
